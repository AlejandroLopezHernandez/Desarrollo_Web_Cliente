...
function añadir_botones_desplegables(){
  contenido = "";
  contenido += '<select id="n_productos" class="cart-button">';
  ...
  contenido += '<select id="select_precio" class="cart-button">';
  contenido += '  <option>Sin orden</option>';
  contenido += '  <option id="p_mayor">Ordenar por precio mayor</option>';
  contenido += '  <option id="p_menor">Ordenar por precio menor</option>';
  contenido += '  <option id="descuento">Ordenar por descuento</option>';
  contenido += '</select> <br>';

document.innerHTML += contenido;
}

function ordenar_x_cantidad(){
  let numero_productos = document.getElementById('n_productos').value;
  if (numero_productos !== "todos"){
    numero_productos = parseInt(numero_productos);
  }
    let limite;
    if(numero_productos === "todos"){
      limite = inventario.length;
    } else {
      limite = Math.min(valor_productos, inventario.length);
    }
    let contenido = "";
    let indice = 0;
    for (let i = 0; i < limite; i ++){
        ...
        contenido += '    <img src="' + inventario[i]['url_imagen'] + '" alt="' + inventario[i]['descripcion'] + '" class="img-fluid">';
        contenido += '  </div>';
        contenido += '  <div class="col-md-8">';
        contenido += '    <h5>' + inventario[i]['descripcion'] + '</h5>';
        contenido += '    <p>Precio: $' + inventario[i]['precio'] + '</p>';
        contenido += '    <p>Existencias: ' + inventario[i]['stock'] + '</p>';
...
function funcion_comprar_carrito(){
  if(carrito.length > 0){
    ...
    if(!document.getElementById("finalizar_compra")){
      ...
      let seleccionarCantidad = document.querySelector('#n_productos');
      let seleccionarPrecio = document.querySelector('#select_precio');

      document.body.removeChild(seleccionarCantidad);
      document.body.removeChild(seleccionarPrecio);
    }
      }
   else {
